...
def change_password(request):
    if request.method =='POST':
        ...
        if new != new1:
            messages.warning(request, 'New password and conform password shold be same!')
            return redirect('userprofile')
...
def cart_view(request):
    if request.user.is_authenticated:
        ...
        cart, _ = Cart.objects.get_or_create(is_paid = False, session_id = session_key(request))
...
def add_to_cart_from_wish(request,uid):
    ...
    if wish.variant.quantity<1:
        messages.warning(request,'Out of Stock!')
        return HttpResponseRedirect(request.path_info)
    cart_item,created = CartItem.objects.get_or_create(cart=cart, product=wish.product,variant=wish.variant)
